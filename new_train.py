...
class NeuralNetwork:
    ...
    def fit_with_gradient_type(self, X_train, Y_train,
        ...
        for epoch in range(epochs + 1):
            ...
            # Learning rate decay (multiplicative decay per epoch)
            self.learning_rate = self.initial_learning_rate / (1.0 + 0.001 * epoch)
...
def train_until_accuracy(model, X_train, Y_train,
    ...
    for epoch in range(max_epochs + 1):
        ...
        # LR decay (multiplicative decay per epoch)
        model.learning_rate = model.initial_learning_rate / (1.0 + 0.001 * epoch)
...
if __name__ == "__main__":
    ...
    data_path = "new_clean.csv"  
    data = pd.read_csv(data_path).to_numpy()
    np.random.shuffle(data)  # สุ่มลำดับแถว
    
    m, n = data.shape  # m = จำนวนตัวอย่าง, n = 1(label) + 784(pixel)
    
    print(f"Total data points: {m}")
    print(f"Features per sample: {n}")
    
    # dev/test = 1000 ตัวแรก (or all if less than 1000)
    dev_size = min(1000, m // 2)
    ...
    # Validate data
    if X_train.shape[1] == 0 or X_dev.shape[1] == 0:
        print("Error: No training or dev data available!")
        exit(1)
    
    # ----- 2. สร้างโมเดล -----
    input_size = X_train.shape[0]           # 784
    num_classes = len(np.unique(Y_train))   # ควรได้ 10
